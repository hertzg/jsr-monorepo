 ...
 * For the IPv4 counterpart see `ipv4ToArpa`, and for one that takes either
 * version see `ipToArpa`.
 *
 ...
 * value. `ipToArpa` never sees one, since `parseIp` unwraps mapped addresses
 * to IPv4 (see ADR 0004).
 *
 * @param address The address as a 128-bit bigint
 ...
 * ```ts
 * import { assertEquals } from "@std/assert";
 * import { ipv6ToArpa, parseIpv6 } from "@hertzg/ip/ipv6";
 *
 * assertEquals(
