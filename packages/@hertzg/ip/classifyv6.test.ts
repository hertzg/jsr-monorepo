...
Deno.test("isAddressv6Loopback", async (t) => {
  ...
  await t.step("rejects non-loopback", () => {
    ...
    assertEquals(isAddressv6Loopback(parseAddressv6("2001:db8::1")), false);
  });
});
...
Deno.test("isAddressv6Unspecified", async (t) => {
  ...
  await t.step("rejects non-unspecified", () => {
    ...
    assertEquals(isAddressv6Unspecified(parseAddressv6("2001:db8::1")), false);
  });
});
...
Deno.test("isAddressv6LinkLocal", async (t) => {
  await t.step("matches fe80::/10", () => {
    ...
    assert(
      isAddressv6LinkLocal(
        parseAddressv6("febf:ffff:ffff:ffff:ffff:ffff:ffff:ffff"),
      ),
    );
  });

  await t.step("rejects non-link-local", () => {
    assertEquals(isAddressv6LinkLocal(parseAddressv6("fe7f::")), false);
    assertEquals(isAddressv6LinkLocal(parseAddressv6("fec0::")), false);
  });
});
...
Deno.test("isAddressv6Multicast", async (t) => {
  ...
  await t.step("rejects non-multicast", () => {
    assertEquals(isAddressv6Multicast(parseAddressv6("feff::")), false);
...
Deno.test("isAddressv6UniqueLocal", async (t) => {
  await t.step("matches fc00::/7", () => {
    assert(isAddressv6UniqueLocal(parseAddressv6("fc00::")));
    assert(isAddressv6UniqueLocal(parseAddressv6("fc00::1")));
    assert(isAddressv6UniqueLocal(parseAddressv6("fd00::1")));
    assert(
      isAddressv6UniqueLocal(
        parseAddressv6("fdff:ffff:ffff:ffff:ffff:ffff:ffff:ffff"),
      ),
    );
  });

  await t.step("rejects non-unique-local", () => {
    assertEquals(isAddressv6UniqueLocal(parseAddressv6("fbff::")), false);
    assertEquals(isAddressv6UniqueLocal(parseAddressv6("fe00::")), false);
  });
});

Deno.test("isAddressv6GlobalUnicast", async (t) => {
  await t.step("matches 2000::/3", () => {
    assert(isAddressv6GlobalUnicast(parseAddressv6("2000::")));
    assert(isAddressv6GlobalUnicast(parseAddressv6("2001:db8::1")));
    assert(
      isAddressv6GlobalUnicast(parseAddressv6("2607:f8b0:4004:800::200e")),
    );
    assert(
      isAddressv6GlobalUnicast(
        parseAddressv6("3fff:ffff:ffff:ffff:ffff:ffff:ffff:ffff"),
      ),
    );
  });

  await t.step("rejects non-global-unicast", () => {
    assertEquals(isAddressv6GlobalUnicast(parseAddressv6("1fff::")), false);
    assertEquals(isAddressv6GlobalUnicast(parseAddressv6("4000::")), false);
...
Deno.test("isAddressv6Mapped", async (t) => {
  await t.step("matches ::ffff:0:0/96", () => {
    assert(isAddressv6Mapped(parseAddressv6("::ffff:0.0.0.0")));
    assert(isAddressv6Mapped(parseAddressv6("::ffff:192.168.1.1")));
    assert(isAddressv6Mapped(parseAddressv6("::ffff:255.255.255.255")));
  ...
  await t.step("rejects non-ipv4-mapped", () => {
    ...
    assertEquals(isAddressv6Mapped(parseAddressv6("::fffe:c0a8:101")), false);
  });
});
...
Deno.test("isAddressv6Translated", async (t) => {
  await t.step("matches 64:ff9b::/96", () => {
    ...
    assert(isAddressv6Translated(parseAddressv6("64:ff9b::ffff:ffff")));
  });

  await t.step("rejects non-ipv4-translated", () => {
    assertEquals(isAddressv6Translated(parseAddressv6("64:ff9a::")), false);
    assertEquals(isAddressv6Translated(parseAddressv6("65:ff9b::")), false);
  });
});

Deno.test("isAddressv6Documentation", async (t) => {
  await t.step("matches 2001:db8::/32", () => {
    assert(isAddressv6Documentation(parseAddressv6("2001:db8::")));
  ...
  await t.step("rejects non-documentation", () => {
    assertEquals(isAddressv6Documentation(parseAddressv6("2001:db7::")), false);
    assertEquals(isAddressv6Documentation(parseAddressv6("2001:db9::")), false);
  });
});

Deno.test("isAddressv6Teredo", async (t) => {
  await t.step("matches 2001::/32", () => {
    assert(isAddressv6Teredo(parseAddressv6("2001::")));
    assert(isAddressv6Teredo(parseAddressv6("2001::1")));
    assert(
      isAddressv6Teredo(parseAddressv6("2001:0:ffff:ffff:ffff:ffff:ffff:ffff")),
    );
  });

  await t.step("rejects non-teredo", () => {
    assertEquals(isAddressv6Teredo(parseAddressv6("2000:ffff::")), false);
    assertEquals(isAddressv6Teredo(parseAddressv6("2002::")), false);
  });
});

Deno.test("isAddressv6Benchmarking", async (t) => {
  await t.step("matches 2001:2::/48", () => {
    assert(isAddressv6Benchmarking(parseAddressv6("2001:2::")));
    assert(isAddressv6Benchmarking(parseAddressv6("2001:2::1")));
    assert(
      isAddressv6Benchmarking(
        parseAddressv6("2001:2:0:ffff:ffff:ffff:ffff:ffff"),
      ),
    );
  });
  ...
  await t.step("rejects non-benchmarking", () => {
    assertEquals(
      isAddressv6Benchmarking(parseAddressv6("2001:1:ffff::")),
      false,
    );
    assertEquals(isAddressv6Benchmarking(parseAddressv6("2001:3::")), false);
  });
});

Deno.test("isAddressv6Orchidv2", async (t) => {
  await t.step("matches 2001:20::/28", () => {
    assert(isAddressv6Orchidv2(parseAddressv6("2001:20::")));
    assert(isAddressv6Orchidv2(parseAddressv6("2001:20::1")));
    assert(
      isAddressv6Orchidv2(
        parseAddressv6("2001:2f:ffff:ffff:ffff:ffff:ffff:ffff"),
      ),
    );
  });

  await t.step("rejects non-orchidv2", () => {
    assertEquals(isAddressv6Orchidv2(parseAddressv6("2001:1f:ffff::")), false);
    assertEquals(isAddressv6Orchidv2(parseAddressv6("2001:30::")), false);
  });
});
...
Deno.test("classifyAddressv6", async (t) => {
  await t.step("classifies all range types", () => {
    ...
    assertEquals(
      classifyAddressv6(parseAddressv6("::ffff:192.168.1.1")),
      "ipv4-mapped",
    );
    assertEquals(
      classifyAddressv6(parseAddressv6("64:ff9b::1")),
      "ipv4-translated",
    );
    assertEquals(
      classifyAddressv6(parseAddressv6("2001:db8::1")),
      "documentation",
    );
    assertEquals(
      classifyAddressv6(parseAddressv6("2001:2::1")),
      "benchmarking",
    );
    ...
    assertEquals(
      classifyAddressv6(parseAddressv6("2607:f8b0:4004:800::200e")),
      "global-unicast",
    );
  });

  await t.step("returns unassigned for unknown ranges", () => {
    assertEquals(classifyAddressv6(parseAddressv6("100::")), "unassigned");
    assertEquals(classifyAddressv6(parseAddressv6("4000::")), "unassigned");
  });
